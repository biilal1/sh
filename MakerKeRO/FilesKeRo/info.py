...
from FilesKeRo.Data import get_data
from googletrans import Translator
from FilesKeRo.Data import (get_call, get_app, get_userbot, get_group, get_channel, must_join)
...
KeRo = "https://raw.githubusercontent.com/KeroIsDev/keroooo/refs/heads/main/IMG_20240921_201153_186.jpg"
...
async def download(bot_username, link, video: Union[bool, str] = None):
        ...
        def audio_dl():
            ydl_optssx = {"format": "bestaudio/best", "outtmpl": f"downloads/%(id)s.%(ext)s", "geo_bypass": True, "nocheckcertificate": True, "quiet": True, "no_warnings": True,"merge_output_format": "mp3", "username": "oauth2", "password" : ""}
...
async def change_stream(bot_username, client, chat_id):
           try:
            ...
            if videoid:
              KeRo = await app.get_chat(OWNER[0])
              photo_id = KeRo.photo.big_file_id
...
async def joinch(message):
        ...
        ch = cch.replace("https://t.me/", "OE_YT")
        try:
            await message._client.get_chat_member(ch, message.from_user.id)
        except UserNotParticipant:
            try:
                await message.reply(f"**يجب ان تشترك في القناة.\n\nقنـاة الـبـوت ⟨ {cch} ⟩.**",disable_web_page_preview=True,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("اضـغط هنا للأشتـراك القنـاة.", url=f"{cch}"),],]))
                return True
