...
async def get_userbot(bot_username):
  ...
  if not userbot:
   ...
   for i in Bots:
       ...
       if bot == bot_username:
         session = i["session"]
         userbot = Client("FilesKeRo", api_id=API_ID, api_hash=API_HASH, session_string=session)
...
async def get_app(bot_username):
  ...
  if not app:
   ...
   for i in Bots:
       ...
       if bot == bot_username:
         token = i["token"]
         app = Client("FilesKeRo", api_id=API_ID, api_hash=API_HASH, bot_token=token, plugins=dict(root="FilesKeRo"))
