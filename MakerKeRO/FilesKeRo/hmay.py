...
from OWNER import PHOTO , KeroId
...
async def lllocj7865j(client, message):
    ...
    if chek.status == ChatMemberStatus.OWNER or message.from_user.id == OWNER_ID or message.from_user.username in OWNER:
...
@Client.on_message(filters.command(["قفل السب"], "") & filters.group, group=1234566)
async def lockKero(client, message):
    bot_username = client.me.username
    OWNER_ID = await get_dev(bot_username)
    group_id = message.chat.id
    chek = await client.get_chat_member(message.chat.id, message.from_user.id)
...
@Client.on_message(filters.command(["فتح السب"], "") & filters.group, group=12345667)
async def UnlockKero(client, message):
    bot_username = client.me.username
    OWNER_ID = await get_dev(bot_username)
    group_id = message.chat.id
    chek = await client.get_chat_member(message.chat.id, message.from_user.id)
...
@Client.on_message(group=123456678)
...
async def tasfaya(client, message):
    ...
    if chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == OWNER_ID or message.from_user.username in OWNER:
        if message.chat.id in mannof:
            return await message.reply_text(f"عذرا عزيزي [{message.from_user.mention}] الامر معطل من قبل مالك الجروب ✨✅")
        OWNERar = await client.get_chat(message.from_user.id)
        CASER = OWNERar.first_name
...
async def OWNERarprivileges(client, message):
    bot_username = client.me.username
    OWNER_ID = await get_dev(bot_username)
    if await set_join_must(client, message):
     return
