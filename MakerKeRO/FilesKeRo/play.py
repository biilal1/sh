...
from FilesKeRo.info import (db, add, is_served_call, add_active_video_chat, add_served_call, add_active_chat, gen_thumb, download, remove_active, joinch)
from FilesKeRo.Data import (get_logger, get_userbot, get_call, get_logger_mode, get_group, get_channel)
import asyncio
...
async def play(client: Client, message):
  ...
  user_id = message.from_user.id if message.from_user else "Y_J_J_J"
  message_id = message.id 
  gr = await get_group(bot_username)
  ch = await get_channel(bot_username)
  button = [[InlineKeyboardButton(text="𓏺 َِ𝖤َِ𝗇َِ𝖣 .", callback_data=f"stop"), InlineKeyboardButton(text="𓏺 َِ𝖱َِ𝖾َِ𝖲َِ𝗎َِ𝖬َِ𝖾 .", callback_data=f"resume"), InlineKeyboardButton(text="𓏺 َِ𝖯َِ𝖺َِ𝖴َِ𝗌َِ𝖤 .", callback_data=f"pause")], [InlineKeyboardButton(text=f"{OWNER_NAME}", url=f"https://t.me/Y_J_J_J")], [InlineKeyboardButton(text="اضف البوت الي مجموعتك او قناتك ⚡", url=f"https://t.me/{bot_username}?startgroup=True")]]
     ...
     if await is_served_call(client, message.chat.id):
         ...
         if message.from_user:
          if message.from_user.photo:
           ...
           ahmed = await client.get_chat("Y_J_J_J")
