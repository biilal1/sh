...
from FilesKeRo.Data import get_dev, get_group, get_channel, get_dev_name
...
async def arbic(client: Client, query: CallbackQuery):
    ...
    await query.edit_message_text(f"**{query.from_user.mention} : مرحباً بك عزيزي **\n\n**انا بوت تشغيل موسيقى صوتية ومرئية .⚡**\n**قم بإضافة البوت إلي مجموعتك او قناتك .⚡**\n**سيتم تفعيل البوت وانضمام المساعد تلقائياً**\n**في حال مواجهت مشاكل انضم هنا **\n**@ **\n**استخدم الازرار لمعرفه اوامر الاستخدام .⚡ **",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("اضف البوت اللي مجموعتك.",url=f"https://t.me/{bot.username}?startgroup=true",)
                ],
                [
                    InlineKeyboardButton("اوامر الحماية", callback_data="ProtectKeroAr"),
                ],
                [
...
Powered By [{OWNER_NAME}]""",
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton("Cmd Protect", callback_data="ProtectKeroEn") ,
                ],
                [
                    InlineKeyboardButton("Cmd Play", callback_data="cbcmds"),
                    InlineKeyboardButton("Cmd Activate", callback_data="cbhowtouse"),
                ],
                [
                    InlineKeyboardButton(
                        "Group", url=f"{gr}"
...
@Client.on_callback_query(filters.regex("ProtectKeroAr"))
async def ProtectKeroAr(_, query: CallbackQuery):
    await query.edit_message_text(
f"""
**اوامر الحماية في البوت ⚡♥️:**
1.) **منع تصفيه تلقائي ⚡.**
2.) **قفل الروابط - التوجيه ⚡.**
3.) **قفل السب مفعل تلقائي ⚡.**
3.) **قفل الصور - قفل الفيديو ⚡.**
4.) **قفل المنشن - قفل الملصقات ⚡.**
5.) **لعرض قائمه الحمايه اكتب الحماية ⚡.**
💡 **في حال واجهت اي مشكلة اخري يمكنك التواصل مع المطور من هن : {OWNER_NAME} **
""",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("العودة", callback_data="arbic")]]
        ),
...
@Client.on_callback_query(filters.regex("ProtectKeroEn"))
async def ProtectKeroEn(_, query: CallbackQuery):
    await query.edit_message_text(
f"""
**Protection commands in the bot ⚡♥️:**
1.) **Prevent automatic filtering ⚡.**
2.) **Lock links - directives ⚡.**
3.) **Lock the insult automatically activated ⚡.**
3.) **Lock images - lock videos ⚡.**
4.) **Lock mentions - lock stickers ⚡.**
5.) **To display the protection list, type protection ⚡.**
💡 **If you encounter any other problem, you can contact the developer from here: {OWNER_NAME} **
""",
        reply_markup=InlineKeyboardMarkup(
