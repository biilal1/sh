...
from FilesKeRo.info import Call, activecall, helper, active
from FilesKeRo.Data import db, dev, devname, set_must, get_data
  ...
  for i in bots:
      ...
      try:
       if not i["bot_username"] in Done:
        ...
        logger = i["logger"]
        bot = Client("FilesKeRo", api_id=API_ID, api_hash=API_HASH, bot_token=TOKEN, in_memory=True, plugins=dict(root="FilesKeRo"))
        user = Client("FilesKeRo", api_id=API_ID, api_hash=API_HASH, session_string=SESSION, in_memory=True)
        await bot.start()
        await user.start()
...
["تفعيل الاشتراك الإجباري", "تعطيل الاشتراك الإجباري"],
["حظر بوت", "حظر مستخدم"],
